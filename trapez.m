...
%   For matrices, TRAPZ(Y) is a row vector with the integral over each column. 
%   For N-D arrays, TRAPZ(Y) works across the first non-singleton dimension.
%
...
%   The length of X must be the same as size(Y,DIM)).
%
%   Example:
%       Y = [0 1 2; 3 4 5]
%       trapz(Y,1)
